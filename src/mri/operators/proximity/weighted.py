...
class WeightedSparseThreshold(SparseThreshold):
    ...
    use_gpu: bool, default False
        If True, the computation is done on GPU, input should be cupy arrays
    ...
    def __init__(self, weights, coeffs_shape, weight_type='scale_based',
                 zero_weight_coarse=True, linear=Identity(), use_gpu=False, **kwargs):
    ...
    def mu(self, w):
        ...
        if self.use_gpu:
            try:
                ...
                raise ImportError('Cupy is not installed, cannot run on GPU')
            xp = cp
        else:
            xp = np
        weights_init = xp.zeros(np.sum(np.prod(self.cf_shape, axis=-1)))
        start = 0
        if self.weight_type == 'scale_based':
            scale_shapes = np.unique(self.cf_shape, axis=0)
            num_scales = len(scale_shapes)
            if isinstance(w, (float, int, xp.float64)):
                weights = w * xp.ones(num_scales)
            else:
                if len(w) != num_scales:
            ...
            for i, scale_shape in enumerate(np.unique(self.cf_shape, axis=0)):
                scale_sz = xp.prod(scale_shape)
                stop = start + scale_sz * xp.sum(scale_shape == self.cf_shape)
                weights_init[start:stop] = weights[i]
                start = stop
        elif self.weight_type == 'custom':
            if isinstance(w, (float, int, xp.float64)):
                w = w * xp.ones(weights_init.shape[0])
            weights_init = w
